...
from climtech.config.telemetry.provider import DifferentSamplerPerLibraryTracerProvider
from climtech.config.telemetry.utils import BatchBaggageSpanProcessor, otel_is_enabled
...
def setup_logging():
    ...
    logger.add(
        sys.stderr, format=loguru_format, level=settings.CLIMTECH_LOG_LEVEL
    )
...
def setup_telemetry(add_django_instrumentation: bool):
    ...
    if otel_is_enabled():
        ...
        print("Not configuring telemetry due to CLIMTECH_ENABLE_OTEL not being set.")
...
def _setup_log_exporting(logger):
    ...
    handler = LogGuruCompatibleLoggerHandler(
        level=settings.CLIMTECH_LOG_LEVEL,
        logger_provider=logger_provider,
    )
    logger_provider.add_log_record_processor(BatchLogRecordProcessor(exporter))
    logger.add(handler, format="{message}", level=settings.CLIMTECH_LOG_LEVEL)

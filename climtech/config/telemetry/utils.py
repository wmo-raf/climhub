...
# climtech manually.
CLIMTECH_OTEL_TRACE_ATTR_PREFIX = "climtech."
...
def _climtech_trace_func(wrapped_func, tracer: Tracer):
...
def add_climtech_trace_attrs(**kwargs):
    """
...
def otel_is_enabled():
    env_var_set = bool(os.getenv("CLIMTECH_ENABLE_OTEL", False))
    not_in_tests = (
            os.getenv("DJANGO_SETTINGS_MODULE", "").strip()
            != "climtech.config.settings.test"
    )
